$(document).ready(function() {
	...
	$local.tablaPartes.$th_tipo_autorizacion.text("Respuesta");
	...
	$local.$buscar.on("click", function() {
		...
		$.ajax({
			type : "GET",
			url : $variableUtil.root + "reporte/autorizacion/respuestaSwitch?accion=buscar",
			...
			success : function(reporteRespuestaSwitch) {
				if (reporteRespuestaSwitch.length == 0) {
				...
				$funcionReporteUtil.llenarTabla(reporteRespuestaSwitch, columnas_tr_mes_o_dia, criterioBusquedaAutorizacion.codigoRespuestaTransaccion, $local.$tablaResultadoBusquedaCuerpo, $local.tablaPartes.$tr_foot_total, cant_columna_por_tr_mes_o_dia);
