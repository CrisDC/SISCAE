$(document).ready(function() {
	...
	$local.tablaPartes.$th_tipo_autorizacion.text("Id Proceso");
	...
	$local.$buscar.on("click", function() {
		...
		$.ajax({
			type : "GET",
			url : $variableUtil.root + "reporte/autorizacion/procesoSwitch?accion=buscar",
			...
			success : function(reporteProcesoSwitch) {
				if (reporteProcesoSwitch.length == 0) {
				...
				$funcionReporteUtil.llenarTabla(reporteProcesoSwitch, columnas_tr_mes_o_dia, criterioBusquedaAutorizacion.codigoRespuestaTransaccion, $local.$tablaResultadoBusquedaCuerpo, $local.tablaPartes.$tr_foot_total, cant_columna_por_tr_mes_o_dia);
