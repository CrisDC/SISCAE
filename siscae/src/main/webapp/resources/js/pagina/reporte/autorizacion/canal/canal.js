$(document).ready(function() {
	...
	$local.tablaPartes.$th_tipo_autorizacion.text("Canal");
	...
	$local.$buscar.on("click", function() {
		...
		$.ajax({
			type : "GET",
			url : $variableUtil.root + "reporte/autorizacion/canal?accion=buscar",
			...
			success : function(reporteCanal) {
				if (reporteCanal.length == 0) {
				...
				$funcionReporteUtil.llenarTabla(reporteCanal, columnas_tr_mes_o_dia, criterioBusquedaAutorizacion.codigoRespuestaTransaccion, $local.$tablaResultadoBusquedaCuerpo, $local.tablaPartes.$tr_foot_total, cant_columna_por_tr_mes_o_dia);
