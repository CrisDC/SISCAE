$(document).ready(function() {
	...
	$local.$buscar.on("click", function() {
		...
		$.ajax({
			type : "GET",
			url : $variableUtil.root + "reporte/tarifario/detalleEmisor?accion=buscar",
			...
			success : function(reporteTarifarioDetalleEmisor) {
				if (reporteTarifarioDetalleEmisor.length == 0) {
					$funcionUtil.notificarException($variableUtil.busquedaSinResultados, "fa-exclamation-circle", "Información", "info");
					return;
				}
				$local.tablaResultadoBusqueda.rows.add(reporteTarifarioDetalleEmisor).draw();
			},
			error : function() {
			},
			complete : function() {
	...
	$local.$exportar.on("click", function() {
		...
		window.location.href = $variableUtil.root + "reporte/tarifario/detalleEmisor?accion=exportar&" + paramCriterioBusqueda;
	});
