...
public @RestController class MembresiaController
{
    ...
    @PostMapping
    public ResponseEntity<?> registrarMembresia(
            @Validated({ Default.class, IRegistro.class }) @RequestBody Membresia membresia,
            Errors error)
    {
        if (error.hasErrors())
        {
    ...
    public ResponseEntity<?> actualizarMembresia(
            @Validated({ Default.class, IActualizacion.class }) @RequestBody Membresia membresia,
            Errors error)
    {
        if (error.hasErrors())
        {
    ...
    @DeleteMapping
    public ResponseEntity<?> eliminarMembresia(
            @Validated(IActualizacion.class) @RequestBody Membresia membresia, Errors error)
    {
        if (error.hasErrors())
        {
            throw new BadRequestException(ValidatorUtil.obtenerMensajeValidacionError(error));
